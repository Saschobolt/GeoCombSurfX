...
function contacts(poly1::AbstractPolyhedron, poly2::AbstractPolyhedron; atol = 1e-8)
    ...
    poly_facets1 = [Polyhedra.polyhedron(Polyhedra.vrep(transpose(hcat(coords...)))) for coords in coords_facets1]
    poly_facets2 = [Polyhedra.polyhedron(Polyhedra.vrep(transpose(hcat(coords...)))) for coords in coords_facets2]

    intersections = Matrix{Vector{Vector{<:Real}}}([[Polyhedra.vrep(Polyhedra.intersect(face1, face2)).V[k,:] for k in 1:size(Polyhedra.vrep(Polyhedra.intersect(face1, face2)).V)[1]] for face1 in poly_facets1, face2 in poly_facets2])
